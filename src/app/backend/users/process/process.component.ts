...
export class ProcessComponent implements OnInit, OnDestroy {
    ...
    onSubmit() {
        if (this.fm.valid && this.flags) {
            ...
            if(this.id === 0) {
                const permissionFull = JSON.stringify(AUTHORIZATION_LIST.map((item) => (item.key)));
                const permissionDashboard = JSON.stringify([AUTHORIZATION_LIST[0].key]);
                data.permission = permissionDashboard;
            };
