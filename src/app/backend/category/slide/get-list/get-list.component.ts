...
export class GetlistComponent implements OnInit, OnDestroy {
    ...
    private conf = {
        getlist: {
            path: "settings/sliders/getlist",
            token: 'getListSliders'
        },
        changestatus: {
            path: "settings/sliders/changestatus",
            token: 'changestatus'
        },
        remove: {
            path: "settings/sliders/remove",
            token: 'removeSliders'
        }
    }
    ...
    onRemove(item: any): void {
        this.id = item.id;
        this.modalRef = this.modalService.show(AlertComponent, { initialState: { messages: 'settings.slide.remove', name: item.name } });
        this.modalRef.content.onClose.subscribe(result => {
            if (result == true) {
                ...
                this.globals.send({ path: this.conf.remove.path, token: this.conf.remove.token, data: data, params: { id: item.id } });
            }
        });
    }
