...
export class GetlistComponent implements OnInit, OnDestroy {
    ...
    private conf = {
        ...
        changestatus: {
            ...
            token: 'changestatustags'
        },
        remove: {
            path: "settings/tags/remove",
            token: 'removetags'
        }
    }
    private cols = [
    ...
    onRemove(item: any): void {
        this.id = item.id;
        this.modalRef = this.modalService.show(AlertComponent, { initialState: { messages: 'settings.tags.remove', name: item.name } });
